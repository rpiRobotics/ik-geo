...
% subproblem.sp_5  Subproblem 5: Three circles
...
%   If they are, rewrite the problem and solve using subproblems 1-4
%
%   The problem is ill-posed if (k1, p1), or (k3, p3) are parallel
%
%   Inputs:
%       p0: 3x1 vector
%       p1: 3x1 vector
...
for i_z = 1:length(z_vec)
    ...
    for i_sign = 1:4
        ...
        if abs(norm(Rp_1) - norm(Rp_3)) < 1E-6
            i_soln = 1 + i_soln;
            theta1(i_soln) = atan2(sc1(1), sc1(2));
            theta2(i_soln) = subproblem.sp_1(Rp_3, Rp_1, k2);
