...
def sp5_run(p0, p1, p2, p3, k1, k2, k3):
   ...
   for i_H in range(len(H_vec)):
      ...
      pm_1 = k2 @ A1 @ J * np.sqrt(np.linalg.norm(k2 @ A1, 2)*np.linalg.norm(k2 @ A1, 2) - (H-delta1)*(H-delta1))
      pm_3 = k2 @ A3 @ J * np.sqrt(np.linalg.norm(k2 @ A3, 2)*np.linalg.norm(k2 @ A3, 2) - (H-delta3)*(H-delta3))
